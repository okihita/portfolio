...
- *EngageMedia Digital Rights Suite:* Architected 2 regional civic-tech AI platforms across 11 Southeast Asian nations—building *D.R.O.N.E.* (`drone.okihita.com` --- ongoing development AI policy observatory with Supabase `pgvector` RAG, Vercel AI SDK v4, and D3 mapping) and *MMAAII* (`mmaaii.engagemedia.org` --- bilingual AI incident & legal regulations matrix on Cloudflare Pages).
- *PAKDE (Solo Creator):* Built a gamified, 100% offline-first hackathon project for Indonesian village cooperatives (Koperasi Desa) using Tauri 2.0 (Rust + React 19), automating SAK EP double-entry accounting for cross-platform Windows/macOS desktop apps (`pakde.vercel.app`).

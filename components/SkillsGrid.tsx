...
export default function SkillsGrid() {
  ...
  return (
    <section id="skills" className="py-20 border-t border-zinc-800/80 bg-[#09090b]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {PORTFOLIO_DATA.skills.map((cat, idx) => (
            <div
              key={cat.title}
              className="craft-card rounded-xl p-6 space-y-5 border border-zinc-800/80"
            >
              <div className="flex items-center gap-2.5 border-b border-zinc-800/80 pb-3">
                {getCategoryIcon(idx)}
                <h3 className="text-base font-semibold text-zinc-100">{cat.title}</h3>
              </div>

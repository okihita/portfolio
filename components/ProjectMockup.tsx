...
export default function ProjectMockup({
  ...
  if (project.mockType === "mobile") {
    return (
      <div className="py-2 flex justify-center">
        <div className="w-full max-w-[280px] sm:max-w-[310px] rounded-[36px] p-3 bg-zinc-200 dark:bg-zinc-900 border-2 border-zinc-300 dark:border-zinc-700/80 shadow-xl dark:shadow-2xl flex flex-col font-sans">
          <div
            className="w-full rounded-[26px] bg-slate-100 dark:bg-[#0c0c10] border border-zinc-300 dark:border-zinc-800 p-3 space-y-3 flex flex-col justify-between overflow-hidden relative min-h-[350px]"
            onMouseEnter={() => setIsPaused(true)}
            onMouseLeave={() => setIsPaused(false)}
          >
            <div className="z-20 bg-slate-100/90 dark:bg-[#0c0c10]/90 backdrop-blur-sm pb-1">
              <div className="flex items-center justify-between text-[10px] text-zinc-500 dark:text-zinc-400 font-mono px-1 pb-1.5 border-b border-zinc-200 dark:border-zinc-800/60">
                <span>09:41</span>
                <div className="w-16 h-2.5 rounded-full bg-zinc-300 dark:bg-zinc-800 mx-auto" />
            ...
            {images && images.length > 0 ? (
              <div className="relative w-full h-[290px] rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-800/60 bg-white dark:bg-black">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={currentIdx}
              ...
              <>
                <div className="pt-2 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{project.title}</div>
                  </div>
                ...
                <div className="space-y-2 font-mono text-[11px]">
                  <div className="p-3 rounded-xl bg-white dark:bg-zinc-900/90 border border-zinc-200 dark:border-zinc-800 space-y-2 shadow-2xs">
                    <div className="flex items-center justify-between">
                      <span className="text-zinc-800 dark:text-zinc-300 font-semibold">Hermes Engine</span>
                      <span className="text-zinc-500 text-[10px]">v1.0.0</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      <div className="p-2 rounded bg-zinc-100 dark:bg-zinc-950 text-zinc-800 dark:text-zinc-300">
                        <div className="text-zinc-500">UI Thread</div>
                        <div className="font-bold text-emerald-600 dark:text-emerald-400">60 FPS</div>
                      </div>
                      <div className="p-2 rounded bg-zinc-100 dark:bg-zinc-950 text-zinc-800 dark:text-zinc-300">
                        <div className="text-zinc-500">Cold Launch</div>
                        <div className="font-bold text-zinc-900 dark:text-zinc-100">&lt; 0.8s</div>
                      </div>
                    </div>
                  </div>

                  <div className="p-2.5 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-between text-blue-600 dark:text-blue-300 text-[11px]">
                    <div className="flex items-center gap-2">
                      <RotateCcw className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
                      <span>Play Store Production</span>
                    </div>
                    <span className="font-semibold text-zinc-900 dark:text-zinc-100">Live</span>
                  </div>
                </div>
              </>
            )}
            ...
            <div className="pt-1 flex items-center justify-around text-zinc-400">
              <div className="w-8 h-1 bg-zinc-400 dark:bg-zinc-600 rounded-full mx-auto" />
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-zinc-300 dark:border-zinc-800 bg-slate-900 dark:bg-zinc-950 shadow-xl flex flex-col font-mono text-xs">
      <div className="bg-slate-800 dark:bg-zinc-900 border-b border-slate-700 dark:border-zinc-800 px-3.5 py-2.5 flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-slate-600 dark:bg-zinc-700 inline-block" />
          <span className="w-2.5 h-2.5 rounded-full bg-slate-600 dark:bg-zinc-700 inline-block" />
          <span className="w-2.5 h-2.5 rounded-full bg-slate-600 dark:bg-zinc-700 inline-block" />
        </div>
        <div className="text-slate-300 dark:text-zinc-400 text-[11px] font-semibold flex items-center gap-1.5">
          <TerminalIcon className="w-3.5 h-3.5 text-amber-400" />
          {project.id}-agent --status
        </div>
        <span className="text-[10px] text-slate-400 dark:text-zinc-500">Python / LLM</span>
      </div>

      <div className="p-4 bg-[#0f172a] dark:bg-[#0a0a0d] text-slate-200 dark:text-zinc-300 space-y-2 min-h-[250px] font-mono leading-relaxed">
        <div className="text-slate-400 dark:text-zinc-500">$ agentic-cli status --active</div>
        <div className="text-emerald-400">✓ Multimodal LLM Agent Engine online. Listening on Webhooks.</div>
        <div className="text-slate-300 dark:text-zinc-400">
          [INFO] WhatsApp Business API connected. Ledger OCR module ready.
        </div>
        <div className="py-2 grid grid-cols-3 gap-2 text-[11px]">
          <div className="p-2 rounded bg-slate-800/80 dark:bg-zinc-900 border border-slate-700 dark:border-zinc-800">
            <div className="text-slate-400 dark:text-zinc-500 text-[10px]">Amartha x GDG</div>
            <div className="font-bold text-slate-100 dark:text-zinc-100 text-sm">Top 5 Team</div>
          </div>
          <div className="p-2 rounded bg-slate-800/80 dark:bg-zinc-900 border border-slate-700 dark:border-zinc-800">
            <div className="text-slate-400 dark:text-zinc-500 text-[10px]">RedAI Tri</div>
            <div className="font-bold text-emerald-400 text-sm">Top 15</div>
          </div>
          <div className="p-2 rounded bg-slate-800/80 dark:bg-zinc-900 border border-slate-700 dark:border-zinc-800">
            <div className="text-slate-400 dark:text-zinc-500 text-[10px]">Avg Latency</div>
            <div className="font-bold text-blue-400 text-sm">&lt; 1.5s</div>
          </div>
        </div>
        <div className="text-slate-300 dark:text-zinc-400 flex items-center gap-2 pt-1">
